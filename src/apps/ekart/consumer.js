...
const prepareEkartData = (ekartDict) => {
  ...
  try {
    ...
    const {sub_reasons=[], event=''} = trackData
    const statusScanType = sub_reasons.length ? `${sub_reasons[0]}_${event}` : event
    ...
    let reasonDict =
      EKART_STATUS_MAPPER[statusType];
    ...
    pickrrEkartDict.track_info =
      trackData["event"].toString();
    pickrrEkartDict.awb = trackData["vendor_tracking_id"].toString();
    pickrrEkartDict.track_location =
      trackData["location"].toString();
