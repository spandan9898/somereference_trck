...
const prepareDelhiveryData = (delhivery_dict) => {
  ...
  try {
    const trackData = delhivery_dict.Shipment;
    ...
    if ("PickUpDate" in trackData) {
      let pickupDate = trackData["PickUpDate"];
      let pickupDatetime;
    ...
    let reasonDict =
      DELHIVERY_NSL_CODE_TO_STATUS_TYPE_MAPPER[delhiveryMapperKey];
    ...
    pickrrDelhiveryDict.track_info =
      trackData["Status"]["Instructions"].toString();
    pickrrDelhiveryDict.awb = trackData["AWB"].toString();
    pickrrDelhiveryDict.track_location =
      trackData["Status"]["StatusLocation"].toString();
    pickrrDelhiveryDict.pickrr_status = PICKRR_STATUS_CODE_MAPPING[statusType];
    pickrrDelhiveryDict.pickrr_sub_status_code =
